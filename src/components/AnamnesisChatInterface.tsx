            ...
            >
              ...
              {msg.executionMetadata && (
                <div className="mt-2 p-2.5 bg-slate-950/90 rounded-xl border border-cyan-500/30 text-[11px] font-mono space-y-1 text-slate-300">
                  ...
                  {/* Google Search Grounding Metadata Display */}
                  {(msg.executionMetadata.groundingMetadata || msg.executionMetadata.externalObservation) && (
                    <div className="pt-2 border-t border-slate-800/80 space-y-1.5">
                      <div className="flex items-center gap-1.5 text-emerald-400 font-semibold text-[11px]">
                        <Globe className="w-3.5 h-3.5 text-emerald-400" />
                        <span>Google Search Grounded Real-Time Data</span>
                      </div>
                      
                      {/* Search Queries */}
                      {msg.executionMetadata.groundingMetadata?.webSearchQueries?.length > 0 && (
                        <div className="flex flex-wrap gap-1 text-[10px]">
                          {msg.executionMetadata.groundingMetadata.webSearchQueries.map((q: string, idx: number) => (
                            <span key={idx} className="px-2 py-0.5 rounded-full bg-emerald-500/10 text-emerald-300 border border-emerald-500/20">
                              "{q}"
                            </span>
                          ))}
                        </div>
                      ...
                      {((msg.executionMetadata.groundingMetadata?.groundingChunks || msg.executionMetadata.groundingMetadata?.searchChunks) || msg.executionMetadata.externalObservation?.results) && (
                        <div className="space-y-1 mt-1">
                          <span className="text-[10px] text-slate-400 block font-medium">Verified Search Sources & Citations:</span>
                          <div className="grid grid-cols-1 gap-1 text-[10px]">
                            {(msg.executionMetadata.groundingMetadata?.groundingChunks || msg.executionMetadata.groundingMetadata?.searchChunks || []).map((chunk: any, cIdx: number) => {
                              ...
                              return (
                                <a
                                  ...
                                  target="_blank"
                                  rel="noreferrer"
                                  className="flex items-center justify-between p-1.5 rounded bg-slate-900 hover:bg-slate-850 text-cyan-300 border border-slate-800 transition-colors group"
                                >
                                  <span className="truncate max-w-[280px] font-medium group-hover:text-cyan-200">{title}</span>
                                  <ExternalLink className="w-3 h-3 text-slate-500 group-hover:text-cyan-400 shrink-0 ml-2" />
                                </a>
                              );
                            })}

                            {/* External retrieval gateway fallback results if any */}
                            {(!msg.executionMetadata.groundingMetadata?.groundingChunks && msg.executionMetadata.externalObservation?.results) && (
                              msg.executionMetadata.externalObservation.results.slice(0, 4).map((r: any, rIdx: number) => (
                                <a
                                  key={rIdx}
                                  href={r.url}
                                  target="_blank"
                                  ...
                                  className="flex items-center justify-between p-1.5 rounded bg-slate-900 hover:bg-slate-850 text-cyan-300 border border-slate-800 transition-colors group"
                                >
                                  <span className="truncate max-w-[280px] font-medium group-hover:text-cyan-200">{r.title}</span>
                                  <ExternalLink className="w-3 h-3 text-slate-500 group-hover:text-cyan-400 shrink-0 ml-2" />
                                </a>
                              ))
                            )}
